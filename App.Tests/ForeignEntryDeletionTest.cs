...
public class ForeignEntryDeletionTest
{
    ...
    public void UserTree()
    {
        ...
        Assert.AreEqual(Access.Users.Read().Where(x => x.ID == -1).Count(), 1, "User couldn't be created");
        
        Access.Allerlinks.Write(new AllerlinkModel() { ID = -1, EntityID = -1});
        Assert.AreEqual(Access.Allerlinks.Read().Where(x => x.ID == -1).Count(), 1, "Allerlink couldn't be created");

        Access.Reservations.Write(new ReservationModel() { ID = -1, UserID = -1 });
        Assert.AreEqual(Access.Reservations.Read().Where(x => x.ID == -1).Count(), 1, "Reservation couldn't be created");

        // user should be deleted
        Assert.IsTrue(Access.Users.Delete(-1), "User couldn't be deleted");

        // all allerlinks of user should be deleted
        Assert.AreEqual(Access.Allerlinks.Read().Where(x => x.EntityID == -1 
            && x.Personal == 1).Count(), 0, "Allerlink still present after user deletion");

        // all reservations should be deleted
        Assert.AreEqual(Access.Reservations.Read().Where(x => x.UserID == -1)
