...
static class ProductLogic
{
    ...
    public static string GetValidNameOrPrice(string type)
    {
        ...
        {
            Console.Clear();
            var productInfo = TerminableUtilsPresent.ReadLine($"Enter {type}: ");
    ...
    public static List<string> GetAllWithinTheme(string? theme)
    {
        ...
        if (theme != "0")
        {
            themeID = ThemeManageLogic.GetThemeIDByName(theme);
        }
        else
        {
            themeID = 0;
    ...
    public static ProductModel? ConvertStringChoiceToProductModel(string productInfo, string type, string name)
    {
        ...
        return Access.Products.Read()
            .FirstOrDefault(p =>
                ...
                (type == "theme" && name != "0" ? p.ThemeID == ThemeManageLogic.GetThemeIDByName(name) : p.ThemeID == themeID));
    }
    ...
    public static void ProductEditValidator(ProductModel oldProduct, string type)
    {
        ...
        if(type == "theme")
        {
            newProductEdit = ThemeValidateLogic.GetValidThemeMenu();
            if (newProductEdit == "REQUEST_PROCESS_EXIT")
                return;
            else
            {
                ThemeID = ThemeManageLogic.GetThemeIDByName(newProductEdit);
            }
        }
        else if(type == "price")
        {
    ...
    lTheme:
        string theme = ThemeValidateLogic.GetValidThemeMenu(); //ask for the theme
        ...
        {
            newProduct.ThemeID = ThemeManageLogic.GetThemeIDByName(theme);
        }
